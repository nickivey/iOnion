...
- (void)viewDidLoad {
    ...
    size.height -= appDelegate.rootController.tabBar.frame.size.height;
    ...
    _toolbar.frame = CGRectMake(0, self.view.frame.size.height - kToolBarHeight - appDelegate.rootController.tabBar.frame.size.height, self.view.frame.size.width, kToolBarHeight);
    ...
    address.font = [UIFont systemFontOfSize:17]; //ADDRESS BAR
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    if ([[[request URL] pathExtension] isEqualToString:@"mp3"]) { //ADD DOWNLOAD TYPES
        _linkMenu = [[UIActionSheet alloc] initWithTitle:nil
                                                delegate:self
                                       cancelButtonTitle:@"Close"
                                  destructiveButtonTitle:@"Download file"
                                       otherButtonTitles:nil, nil];
        [_linkMenu showInView:self.view];
        
        _downloadURL = [request URL];
...
- (void)textFieldDidBeginEditing:(UITextField *)textField {
    if ([_addressField.text isEqualToString:@"Search Bing or enter a address"]) {
        _addressField.text = @"";
    }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (actionSheet == _linkMenu) {
        NSLog(@"Button index %ld", (long)buttonIndex);
        if (buttonIndex == 0) {
            _download = [DownloadTableView sharedManager];
            [_download queueAndStartDownloads:_downloadURL]; //DOO WITH LONG HOLD On LINK
            NSLog(@"%@", _downloadURL);
...
- (void)updateAddress:(NSURLRequest*)request {
    ...
    if ([_addressField.text isEqualToString:@"onionbrowser:home"]) {
        _addressField.text = @"Search Bing or enter a address";
    }
...
- (void)loadAddress:(id)sender event:(UIEvent *)event {
    ...
    if([_addressField.text rangeOfString:dot].location == NSNotFound && !url.scheme){
        NSString *search = [NSString stringWithFormat:@"http://www.bing.com/search?q=%@", _addressField.text];
        NSURL *searchurl = [NSURL URLWithString:search];
        url = searchurl;
        // [self loadURL:url];
    } else if (!url.scheme)
    {
...
-(void)webViewProgress:(NJKWebViewProgress *)webViewProgress updateProgress:(float)progress
{
  ...
  //  self.title = [_myWebView stringByEvaluatingJavaScriptFromString:@"document.title"]; //Sets page title
}
